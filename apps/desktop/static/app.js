...
const state = {
  ...
  activeId: null,
  bridgeReady: false,
  defaultConfig: { theme: 'auto', llmNo: 0, tauRoot: '' },
  modelProfiles: [],
...
function detectStructuredKind(line) {
  ...
  // Tau's ACP bridge currently streams tool calls/results as plain
...
function closeSession(id) {
  ...
  if (sess && sess.bridgeSessionId) {
    const bridgeUrl = window.tau.bridgeUrl || 'http://127.0.0.1:14168';
...
async function getCwd() {
  // Use Tau root as default cwd
  const status = await window.tau.checkStatus();
  return status.tauRoot;
}
...
async function pollSessionMessages(sess) {
  ...
  try {
    while (runtime.busy || runtime.forcePollOnce) {
      ...
      const res = await window.tau.pollSession(sess.bridgeSessionId || sess.id, runtime.lastPolledMessageId || 0);
...
async function sendPrompt(text, images = []) {
  ...
  setBusy(true, 'Thinking…', sess);
  try {
    const res = await window.tau.rpc('session/prompt', {
...
async function cancelPrompt() {
  ...
  try {
    const res = await window.tau.rpc('session/cancel', { sessionId: sess?.bridgeSessionId || state.activeId });
    if (res.error) throw new Error(res.error.message || res.error);
    return true;
  } catch (e) {
  ...
  switch (name) {
    ...
    case 'theme':
      if (['light', 'dark', 'auto'].includes(arg)) {
        ...
        cfg.theme = arg;
        applyTheme();
        await window.tau.saveConfig(cfg);
    ...
    case 'cwd':
      if (!arg) {
        const status = await window.tau.checkStatus();
        showSystem(`cwd: ${sess?.cwd || status.tauRoot}`);
      } else {
        showSystem(`Creating new session in ${arg}…`);
        // Need a new session for different cwd
        const res = await window.tau.rpc('session/new', { cwd: arg, mcp_servers: [] });
...
async function loadModelProfiles() {
  try {
    const result = await window.tau.getModelProfiles();
...
async function restartBridge(options = {}) {
  ...
  await window.tau.startBridge(getActiveConfig().llmNo || 0);
  window.setTimeout(() => {
...
window.tau.onBridgeReady(() => {
  markBridgeReady();
});

window.tau.onBridgeMessage(() => {
...
window.tau.onBridgeError((err) => {
  ...
  if (err.type === 'no-taukey') {
    showError(err.message, 'Setup', async () => {
      await window.tau.openTaukeyTemplate();
    }, { skipDiagnostic: true });
...
window.tau.onBridgeClosed((info) => {
...
$('open-taukey').addEventListener('click', () => openConfigFile(window.tau.openTaukey, '.tau/taukey.py'));
...
(function initSearch() {
  ...
  if (window.tau && window.tau.onOpenSearch) {
    window.tau.onOpenSearch(() => openSearch());
  }
})();
...
(async function init() {
  ...
  const platform = (window.tau && window.tau.platform) || process.platform || 'unknown';
  document.body.classList.add('platform-' + platform);

  try {
    const saved = await window.tau.getConfig();
